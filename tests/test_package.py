...
class PackageTests(unittest.TestCase):
    ...
    def test_archive_is_repeatable_and_has_manifest_at_root(self):
        root=Path(__file__).resolve().parents[1]/'Packages/com.mitsuboshi-studio.animator-make-transition-hotkey'
        with tempfile.TemporaryDirectory() as a,tempfile.TemporaryDirectory() as b:
            m,z,h=build(root,a,'RINYA-Mitsuki/animator-make-transition-hotkey')
            self.assertEqual(h,build(root,b,'RINYA-Mitsuki/animator-make-transition-hotkey')[2])
            with zipfile.ZipFile(z) as archive:
                self.assertEqual(json.loads(archive.read('package.json')),m)
                self.assertIn('Editor/AnimatorMakeTransitionHotkey.cs',archive.namelist())
